...
function validateKeypairs() {
  ...
  for (const key of keypairs) {
    ...
    try {
      const keypair = readKeypair(path);
      log(`✅ ${key}: ${keypair.publicKey.toBase58()}`, colors.green);
    } catch (error) {
...
async function checkAccountBalance(connection: Connection, publicKey: PublicKey, name: string) {
  ...
  if (sol < 5) {
    throw new Error(`Insufficient SOL balance for ${name}: ${sol} SOL (minimum: 5 SOL for mainnet)`);
  }
  
  log(`✅ ${name}: ${sol.toFixed(4)} SOL`, colors.green);
}

async function performSafetyChecks(connection: Connection, deployer: Keypair) {
  log("🔒 Performing safety checks...", colors.yellow);
  
  // Check if program already exists
  const programInfo = await connection.getAccountInfo(PROGRAM_ID);
  if (programInfo) {
    throw new Error("Program already exists on mainnet! Aborting deployment.");
  }
  
  // Check deployer balance
  await checkAccountBalance(connection, deployer.publicKey, "Deployer");
  
  // Check if old mint exists and is valid
  const oldMint = new PublicKey(process.env.OLD_MINT!);
  const oldMintInfo = await connection.getAccountInfo(oldMint);
  if (!oldMintInfo) {
    throw new Error("Old mint does not exist on mainnet!");
  }
  
  log("✅ Safety checks passed", colors.green);
}
...
async function deployProgram(connection: Connection, deployer: Keypair) {
  log("🚀 Deploying program to mainnet...", colors.yellow);
...
async function generateDeploymentReport(
  ...
  const reportPath = `deployment-report-mainnet-${Date.now()}.json`;
...
async function main() {
  try {
    log("🚀 Starting zCoin Mainnet Deployment", colors.bright);
    log("=" * 50, colors.cyan);
    
    // Safety warning
    log("⚠️  WARNING: This will deploy to MAINNET!", colors.red);
    log("⚠️  Make sure you have tested on testnet first!", colors.red);
    log("⚠️  This action cannot be undone!", colors.red);
    log("=" * 50, colors.cyan);
    
    // Confirmation
    const confirmed = await confirmAction("Are you sure you want to deploy to mainnet?");
    if (!confirmed) {
    ...
    // Safety checks
    await performSafetyChecks(connection, deployer);
    
    // Final confirmation
    const finalConfirmed = await confirmAction("Final confirmation - deploy to mainnet?");
    if (!finalConfirmed) {
      log("❌ Deployment cancelled by user", colors.yellow);
      process.exit(0);
    }
    ...
    log("=" * 50, colors.cyan);
    log("🎉 Mainnet Deployment Complete!", colors.green);
    log("=" * 50, colors.cyan);
